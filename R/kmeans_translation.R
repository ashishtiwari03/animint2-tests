...
for (iter in 1:max_iter) {
  ...
  new_centers <- aggregate(
    cbind(x, y) ~ cluster,
    data = data.frame(x = data_points$x, y = data_points$y, cluster = cluster_assign),
    mean
  )
...
cluster_plot <- ggplot() +
  geom_point(
    aes(x = x, y = y, color = cluster, fill = cluster), # FIX: add fill to unify legends
    data = points_iter,
  ...
  geom_point(
    aes(x = x, y = y, color = cluster, fill = cluster), # FIX: same aesthetics as above
    data = centers_iter,
    showSelected = "iteration",
    shape = 21,
    size = 7,
  ...
  scale_color_discrete(guide = "none") + # FIX: show only fill legend, drop duplicate color legend
  theme_large +
  labs(title = "K-means Clustering (Iterations)", fill = "Cluster")

objective_plot <- ggplot(objective_iter, aes(x = iteration, y = total_ss)) +
  geom_line() +
  geom_point(
    clickSelects = "iteration", # FIX: makes points clickable to select iteration
    size = 4
  ) +
  theme_large +
  labs(
    title = "Objective Function (click to select iteration)",
    x = "Iteration",
